...
def do_MD(xB, xC, yC):
	...
	for step in range(NSTEPS):
		...
		F_xB = - K * ( 2 * xB - xC - L0 * ( (xB - xC) / m.sqrt((xC - xB)**2 + yC**2) + 1 ) )
		F_xC = - K * ( 2 * xC - xB - L0 * ( xC / m.sqrt(xC**2 + yC**2) + (xC - xB) / m.sqrt((xC - xB)**2 + yC**2) ) )
		F_yC = - K * ( 2 * yC - L0 * ( yC / m.sqrt(xC**2 + yC**2) + yC / m.sqrt((xC - xB)**2 + yC**2) ) )
